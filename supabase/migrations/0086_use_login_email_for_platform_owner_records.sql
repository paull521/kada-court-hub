-- Existing conference-owner records were backfilled before their login email
-- was available in the application profile. Use the authenticated KCH login
-- email for every linked owner record and for Platform payment read models.
update public.platform_owner_records record
set email=lower(login.email),updated_at=now()
from auth.users login
where login.id=record.profile_id
  and coalesce(trim(login.email),'')<>''
  and record.email is distinct from lower(login.email);
...
begin
  ...
  loop
    ...
    select
      coalesce(owner.full_name,profile.display_name,'Conference Owner') as full_name,
