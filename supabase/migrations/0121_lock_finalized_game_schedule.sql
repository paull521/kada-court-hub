-- Final score records are permanent: no schedule or status change may alter them.
...
begin
  select season.conference_id, season.id, conference.timezone,
         game.home_team_id, game.away_team_id, game.finalized_at
    into v_conference_id, v_season_id, v_timezone,
         v_home_team_id, v_away_team_id, v_finalized_at
  ...
  if v_conference_id is null
    or not public.user_has_conference_role(v_conference_id, array['owner']::public.conference_role[]) then
    raise exception 'Only a conference owner can reschedule games.';
  end if;
  if v_finalized_at is not null then
    raise exception 'A finalized score locks this game’s schedule.';
  end if;
  if v_venue is null or char_length(v_venue) > 120
    or char_length(coalesce(p_court,'')) > 60
  ...
  if exists(
    ...
    where game.id <> p_game_id
      ...
      and (game.starts_at at time zone v_timezone)::date = p_starts_at::date
      and (game.home_team_id in (v_home_team_id, v_away_team_id)
        or game.away_team_id in (v_home_team_id, v_away_team_id))
  ) then
  ...
  if exists(
    ...
    where game.id <> p_game_id
      and game.season_id = v_season_id
      and game.status <> 'canceled'
      and game.starts_at = p_starts_at at time zone v_timezone
      and lower(coalesce(game.court,'')) = lower(coalesce(nullif(trim(p_court),''),''))
  ) then
...
create or replace function public.owner_change_game_status(
  p_game_id uuid,
  p_status text,
  p_reason text default null
)
...
declare
  ...
  v_reason text := nullif(trim(p_reason),'');
begin
  select season.conference_id, game.home_team_id, game.away_team_id, game.finalized_at
    into v_conference_id, v_home_team_id, v_away_team_id, v_finalized_at
  from public.games game
  join public.seasons season on season.id = game.season_id
  where game.id = p_game_id;
  ...
  if p_status not in ('scheduled', 'postponed', 'canceled') then
    raise exception 'Choose a valid game status.';
  end if;
  if p_status <> 'scheduled' and (v_reason is null or char_length(v_reason) > 500) then
    raise exception 'Enter a reason of up to 500 characters.';
  end if;

  update public.games
  set status = p_status,
      status_reason = case when p_status = 'scheduled' then null else v_reason end,
      status_changed_at = now()
  where id = p_game_id;
  ...
  select distinct player.profile_id, 'game_' || p_status,
    case p_status
      ...
      when 'canceled' then 'Game canceled'
      else 'Game rescheduled'
    end,
