-- Use the assigned Div A / Div B roster for the KCH Basketball League demo,
-- even when those test registrations are not marked active. Real conferences
-- continue to bill only active player registrations in active seasons.
...
begin
  ...
  select season.id into v_first_season_id from public.seasons season
  ...
  if v_is_demo then
    select season.id into v_demo_season_id from public.seasons season
    where season.conference_id=p_conference_id and season.canceled_at is null
    order by season.starts_on desc,season.created_at desc limit 1;
  end if;

  for v_season in
    select season.id,season.name,season.starts_on from public.seasons season
    where season.conference_id=p_conference_id and season.archived_at is null and season.canceled_at is null
      and ((season.starts_on<=current_date and season.ends_on>=current_date) or (v_is_demo and season.id=v_demo_season_id))
  ...
  loop
    ...
    where registration.season_id=v_season.id
      and (registration.status='active' or (v_is_demo and registration.team_id is not null));
...
begin
  ...
  select subscription.pilot_season_id,conference.name='KCH Basketball League' into v_pilot_season_id,v_is_demo
  from public.conference_subscriptions subscription join public.conferences conference on conference.id=subscription.conference_id
  where subscription.conference_id=p_conference_id;
  if v_is_demo then select id into v_demo_season_id from public.seasons where conference_id=p_conference_id and canceled_at is null order by starts_on desc,created_at desc limit 1; end if;
  ...
  select coalesce(jsonb_agg(jsonb_build_object('divisionName',division.name,'activePlayers',counts.players,'platformFeeCents',case when season.id=v_pilot_season_id then 0 else counts.players*300 end) order by season.starts_on,division.name),'[]'::jsonb) into v_divisions
  from public.divisions division join public.seasons season on season.id=division.season_id
  left join lateral(select count(*)::integer as players from public.registrations registration where registration.division_id=division.id and (registration.status='active' or (v_is_demo and registration.team_id is not null))) counts on true
  where season.conference_id=p_conference_id and season.archived_at is null and season.canceled_at is null
    and ((season.starts_on<=current_date and season.ends_on>=current_date) or (v_is_demo and season.id=v_demo_season_id));
