-- Suspended and inactive conference players cannot be placed on a team until
-- the owner restores their conference-directory status to Active.
...
begin
  ...
  join public.conference_player_pool pool
    on pool.conference_id = season.conference_id
    and pool.player_id = registration.player_id
  ...
  if v_conference_id is null
    ...
    raise exception 'Only an Active conference player can be assigned to a team.';
  end if;
