...
import {
  ...
  submitTeamRosterAction,
  updateDraftPlayerAction,
...
function DraftedPlayer({
  ...
  teamId: string;
  player: CaptainPortalData["roster"][number];
  locked: boolean;
  detailsOnly?: boolean;
}) {
    ...
    <details className="captain-roster-player">
      ...
      <form action={action} className="owner-form">
        ...
        <div className="compact-fields">
          ...
          <label>
            Uniform size
            <select name="uniformSize" defaultValue={player.uniformSize}>
        ...
        <label>
          Jersey name <small>(optional)</small>
          <input name="jerseyName" maxLength={24} defaultValue={player.jerseyName} />
        </label>
        {detailsOnly && player.role !== "Captain" && (
          <label className="check-row">
            <input
              name="assignCoCaptain"
              type="checkbox"
              defaultChecked={player.role === "Co-captain"}
            />{" "}
            Assign as Co-captain
          </label>
        )}
        <Notice state={state} />
...
export default function CaptainDraftRoster({
  ...
  const [addState, addAction, addPending] = useActionState(saveDraftPlayerAction, initial),
    [submitState, submitAction, submitPending] = useActionState(submitTeamRosterAction, initial);
  const locked = data.draftStatus === "submitted" || data.draftStatus === "approved",
    players = detailsOnly
      ? data.roster
      : data.roster.filter(
          (player) =>
            player.registrationId !== data.activeRegistrationId &&
            !["Captain", "Co-captain"].includes(player.role),
        ),
    availableCandidates = data.candidates.filter(
      (candidate) =>
        !data.roster.some((player) => player.registrationId === candidate.registrationId),
    ),
    atLimit = data.rosterLimit !== null && data.roster.length >= data.rosterLimit;
  const matches =
    query.trim().length < 2
      ? availableCandidates.slice(0, 8)
      : availableCandidates
          .filter((candidate) => candidate.name.toLowerCase().includes(query.trim().toLowerCase()))
          .slice(0, 8);
        ...
        <div className={`captain-draft-status ${data.draftStatus}`}>
          ...
          <span>
            {data.draftStatus === "submitted"
              ? "Locked while the commissioner reviews it."
              : data.draftStatus === "approved"
                ? "Approved and ready for roster publication."
                : data.draftStatus === "changes_requested"
                  ? data.ownerNote ||
                    "The commissioner requested roster changes. Update the players, then resubmit for approval."
                  : "Complete your team, then send it to the commissioner."}
          </span>
        </div>
      )}
        ...
        <div className="captain-roster-list">
          {players.map((player) => (
            <DraftedPlayer
              ...
              teamId={data.teamId}
              player={player}
              locked={detailsOnly ? false : locked}
        ...
        <form action={addAction} className="owner-form captain-player-search">
          ...
          <label>
            ...
            <input
              ...
              placeholder={atLimit ? "Team roster is full" : "Type player name"}
            />
          </label>
            ...
            <div className="leader-search-results">
              {matches.map((candidate) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    setSelected(candidate.invitationId);
          ...
          <div className="compact-fields">
            ...
            <label>
              ...
              <select
                name="position"
                defaultValue={selectedCandidate?.preferredPosition ?? ""}
                key={selectedCandidate?.invitationId ?? "none"}
                disabled={atLimit}
              >
                <option value="">Choose position</option>
                {["G", "SG", "PG", "F", "PF", "C"].map((position) => (
            ...
            <label>
              Uniform size
              <select name="uniformSize" defaultValue="" disabled={atLimit}>
                <option value="">Not set</option>
                {["S", "M", "L", "XL", "2XL", "3XL"].map((size) => (
                  <option key={size} value={size}>
                    {size}
          ...
          <button className="btn primary" disabled={addPending || !selected || atLimit}>
            {addPending ? "Adding…" : "Add to My Team"}
          </button>
        </form>
      )}
      {!locked && !detailsOnly && (
        <form action={submitAction} className="owner-form captain-submit-roster">
          ...
          <button className="btn primary" disabled={submitPending || !players.length}>
            {submitPending
              ? "Sending…"
              : data.draftStatus === "changes_requested"
                ? "Resubmit Updated Roster"
                : "Submit Team Roster to Commissioner"}
          </button>
          <small>
            Submitting locks this roster until the commissioner approves it or requests changes.
          </small>
        </form>
      )}
    </section>
