...
function ScoreGame({game}:{game:OwnerSeason["games"][number]}){
  ...
  return <article id={`score-${game.id}`} className={`score-sheet-card ${unavailable?`game-${game.status}`:""} ${played?"final-locked":""}`}><header><div><b>{game.homeTeam}</b><span>vs {game.awayTeam}</span></div><div className="score-game-meta">{scoreNeeded&&<em className="score-needed">Score needed</em>}<small>{localDate(game.localStartsAt)} · {localTime(game.localStartsAt)}<br/>{game.court||game.venue}</small></div></header>{played?<section className="final-score-lock"><b>FINAL · LOCKED</b><strong>{game.homeScore} – {game.awayScore}</strong><small>Final scores cannot be changed.</small></section>:unavailable?<p className="score-unavailable">{game.status.toUpperCase()}{game.statusReason?` — ${game.statusReason}`:""}. Restore this game from Schedules before posting a result.</p>:<form action={action} className="owner-form score-entry-form"><input type="hidden" name="gameId" value={game.id}/><div className="score-entry-fields"><label><span>{game.homeTeam}</span><input name="homeScore" type="number" min="0" inputMode="numeric" placeholder="0" required readOnly={reviewing}/></label><strong>–</strong><label><span>{game.awayTeam}</span><input name="awayScore" type="number" min="0" inputMode="numeric" placeholder="0" required readOnly={reviewing}/></label></div>{reviewing&&<p className="final-score-warning">Final scores cannot be changed.</p>}{state.error&&<p className="form-error" role="alert">{state.error}</p>}{state.message&&<p className="form-success" role="status">{state.message}</p>}{reviewing?<div className="draft-review-actions"><button type="button" className="btn secondary" onClick={()=>setReviewing(false)}>Go Back</button><button className="btn primary" disabled={pending}>{pending?"Finalizing…":"Final Score"}</button></div>:<button type="button" className="btn primary" onClick={()=>setReviewing(true)}>Review Final Score</button>}</form>}</article>;
}
