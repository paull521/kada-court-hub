...
export default function AppShell({children,active,contexts=[],activeRegistrationId="",notifications=[],profileNeedsAttention=false,paymentNeedsAttention=false,teamHasUnavailable=false,homeHref="/home"}:{children:ReactNode;active:string;contexts?:PlayerContextOption[];activeRegistrationId?:string;notifications?:PlayerNotification[];profileNeedsAttention?:boolean;paymentNeedsAttention?:boolean;teamHasUnavailable?:boolean;homeHref?:string}) {
 return <div className="shell"><header className="topbar"><Image src="/kch-logo.png" alt="KadaCourtHub" width={340} height={130} className="logo" priority/><div className="topbar-actions"><PlayerContextSwitcher contexts={contexts} activeRegistrationId={activeRegistrationId}/><NotificationCenter notifications={notifications}/></div></header><main className="content">{children}</main><nav className="bottom" aria-label="Player navigation">{links.map(([href,key,icon,label])=>{const needsAttention=(key==="profile"&&profileNeedsAttention)||(key==="payments"&&paymentNeedsAttention);return <Link key={key} href={key==="home"?homeHref:href} className={`nav ${active===key?"active":""}`}><b aria-hidden="true">{icon}</b>{label}{key==="team"&&<i className={`nav-team-dot ${teamHasUnavailable?"no":"yes"}`}/>} {needsAttention&&<i className="nav-alert-dot" aria-label="Action needed"/>}</Link>})}</nav></div>;
}
