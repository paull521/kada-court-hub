...
export default function AppShell({children,active,contexts=[],activeRegistrationId="",notifications=[],profileNeedsAttention=false,paymentNeedsAttention=false,teamHasUnavailable=false,homeHref="/home",conferenceName="",headerAction}:{children:ReactNode;active:string;contexts?:PlayerContextOption[];activeRegistrationId?:string;notifications?:PlayerNotification[];profileNeedsAttention?:boolean;paymentNeedsAttention?:boolean;teamHasUnavailable?:boolean;homeHref?:string;conferenceName?:string;headerAction?:ReactNode}) {
 ...
 return <div className="shell"><header className="topbar"><Image src="/kch-logo.png" alt="KadaCourtHub" width={340} height={130} className="logo" priority/><div className="topbar-actions">{headerAction??<PlayerContextSwitcher contexts={contexts} activeRegistrationId={activeRegistrationId}/>}<NotificationCenter notifications={notifications}/></div></header><main className="content">{conferenceName&&<p className="eyebrow">CONFERENCE: {conferenceName}</p>}{children}</main><FastBottomNav items={items} active={active} label="Player navigation"/></div>;
}
