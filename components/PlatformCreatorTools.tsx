...
export function OwnerSubscriptionPayment({conferenceId,billing}:{conferenceId:string;billing:OwnerPaymentBilling}){
 ...
 const playerAccess=currentEntries.filter(entry=>entry.chargeType==="platform_fee").reduce((sum,entry)=>sum+entry.amountCents,0)/100;
 ...
 const pilotSeason=seasonSubscription===0&&playerAccess===0&&billing.divisions.length>0;
 ...
 return <>
  <details className="monthly-subscription-dropdown">
   <summary><span><b>Season Subscription</b><small>{pilotSeason?"Pilot season · no charge":`${activePlayers} active players · ${money(playerAccess)} player access`}{pendingSubmission?` · ${pendingSubmission.method==="zelle"?"Zelle":"Cash"}`:""}</small></span><em className={`owner-subscription-status ${statusClass}`}>{pilotSeason?"Pilot":status}</em><strong>›</strong></summary>
   <div className="monthly-subscription-body">
    <div className="owner-platform-table-wrap"><table><thead><tr><th>ACTIVE DIVISION</th><th>ACTIVE PLAYERS</th><th>PLAYER ACCESS TOTAL</th></tr></thead><tbody>{billing.divisions.length?billing.divisions.map(division=><tr key={division.divisionName}><td>{division.divisionName}</td><td>{division.activePlayers}</td><td>{money(division.platformFeeCents/100)}</td></tr>):<tr><td colSpan={3}>No active divisions yet.</td></tr>}</tbody></table></div>
    <div className="owner-platform-breakdown"><span><small>OWNER COST</small><b>{money(seasonSubscription)}</b></span><span><small>PLAYER COST</small><b>{money(playerAccess)}</b></span><span><small>RECEIVED</small><b>{money(received)}</b></span><span><small>TOTAL</small><b>{money(total)}</b></span></div>
