...
export default function ConferencePlayerInvitation({token}:{token:string}){
  ...
  return <section className="card conference-player-invitation"><div><button type="button" className="btn secondary" onClick={copy}>{copied?"Copied!":"Conference Player Invitation"}</button></div>{qrCode&&<img src={qrCode} alt="Conference invitation QR code"/>}</section>;
}
