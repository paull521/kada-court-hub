...
import {currentContext,currentPlayer,fees,games,roster,type Fee,type Game,type GameResult,type Player,type PlayerView} from "@/lib/data";
...
export type PlayerNotification={id:string;type:string;title:string;body:string;linkPath:string;read:boolean;createdLabel:string};
export type PaymentSubmission={id:string;registrationId:string;feeId:string;amount:number;method:"zelle"|"cash"|"waiver";status:"pending"|"confirmed"|"declined";reference:string;reviewNote:string;createdLabel:string};
export type PaymentHistoryItem={id:string;feeLabel:string;amount:number;method:string;paidLabel:string};
export type PlayerAvailability={registrationId:string;name:string;jerseyNumber:number|null;position:string;role:string;available:boolean;responded:boolean};
export type PlayerPaymentAccount={totalCharges:number;paid:number;waived:number;pending:number;balance:number};
...
export type PlayerTeamInfo={homeUniform:string;awayUniform:string;nextGameUniform:string;darkImage:string;lightImage:string;captain:TeamLeader;coCaptain:TeamLeader;rosterStage:"hidden"|"review"|"final";rosterReviewDeadline:string;divisionRosters:DivisionRosterTeam[]};
...
export async function getPlayerPortalData(): Promise<PlayerPortalData> {
  ...
  try {
    ...
    const activeContext=contexts.find(option=>option.registrationId===requestedRegistrationId)??contexts[0];
    ...
    const [{data:registrations},{data:seasonGameRows},{data:divisionTeamRows},{data:feeRows},{data:invitationRow},{data:notificationRows},{data:submissionRows},{data:uniformSettings},{data:leadershipRows},{data:preferenceRow},{data:rosterBroadcastRows},{data:scheduleWorkflowRow}] = await Promise.all([
      ...
      supabase.from("season_invitations").select("id,response,broadcast_id").eq("registration_id",registration.id).maybeSingle(),
      ...
      supabase.from("division_uniform_settings").select("dark_uniform,light_uniform,dark_image_path,light_image_path").eq("division_id",division.id).maybeSingle(),
      ...
      supabase.from("season_broadcasts").select("broadcast_type,response_deadline").eq("division_id",division.id).in("broadcast_type",["roster_draft","roster_final"]),
    ...
    const publishedDivisionGames=scheduleWorkflowRow?.status==="final"?(seasonGameRows??[]).filter(row=>divisionTeamIds.has(row.home_team_id)&&divisionTeamIds.has(row.away_team_id)&&row.status!=="postponed"&&row.status!=="canceled"):[];
    const mappedGames=(scheduleWorkflowRow?.status==="final"?seasonGameRows??[]:[]).filter(row=>(row.home_team_id===team.id||row.away_team_id===team.id)&&row.status!=="postponed"&&row.status!=="canceled").map(row=>{
      const date=new Date(row.starts_at); const home=row.home_team_id===team.id;
      const parts=new Intl.DateTimeFormat("en-US",{timeZone:timezone,weekday:"short",month:"short",day:"2-digit"}).formatToParts(date);
      const get=(type:Intl.DateTimeFormatPartTypes)=>parts.find(part=>part.type===type)?.value??"";
      const game:Game={id:row.id,day:get("weekday").toUpperCase(),month:get("month").toUpperCase(),date:get("day"),dateLabel:new Intl.DateTimeFormat("en-US",{timeZone:timezone,weekday:"long",month:"short",day:"numeric"}).format(date),time:new Intl.DateTimeFormat("en-US",{timeZone:timezone,hour:"numeric",minute:"2-digit"}).format(date),opponent:teamNames.get(home?row.away_team_id:row.home_team_id)??"Opponent",venue:row.venue,court:row.court??"",side:home?"Home":"Away",uniform:(home?row.home_uniform:row.away_uniform)==="Dark"?"Dark":"White"};
      const teamScore=home?row.home_score:row.away_score;const opponentScore=home?row.away_score:row.home_score;
      return {game,startsAt:date.getTime(),teamScore,opponentScore};
    });
    const now=Date.now();
    const liveGames:Game[]=mappedGames.filter(item=>item.teamScore===null&&item.opponentScore===null&&item.startsAt>=now).map(item=>item.game);
    ...
    const standings:StandingRow[]=(divisionTeamRows??[]).map(standingTeam=>{
      const teamGames=finalDivisionGames.filter(game=>game.home_team_id===standingTeam.id||game.away_team_id===standingTeam.id);
      let wins=0,losses=0,ties=0,pointsFor=0,pointsAgainst=0;
      const outcomes=teamGames.map(game=>{const home=game.home_team_id===standingTeam.id;const scored=home?game.home_score!:game.away_score!;const allowed=home?game.away_score!:game.home_score!;pointsFor+=scored;pointsAgainst+=allowed;if(scored>allowed){wins++;return "W"}if(scored<allowed){losses++;return "L"}ties++;return "T"});
      const latest=outcomes.at(-1);let streakCount=0;for(let index=outcomes.length-1;index>=0&&outcomes[index]===latest;index--)streakCount++;
      return{teamId:standingTeam.id,team:standingTeam.name,played:teamGames.length,wins,losses,ties,pointsFor,pointsAgainst,difference:pointsFor-pointsAgainst,winPercentage:teamGames.length?(wins+ties*.5)/teamGames.length:0,streak:latest?`${latest}${streakCount}`:"—"};
    }).sort((a,b)=>b.winPercentage-a.winPercentage||b.difference-a.difference||b.pointsFor-a.pointsFor||a.team.localeCompare(b.team));
    ...
    const livePaymentHistory:PaymentHistoryItem[]=(paymentRows??[]).map(row=>({id:row.id,feeLabel:feeLabels.get(row.fee_id)??"Account payment",amount:row.amount_cents/100,method:row.method,paidLabel:new Intl.DateTimeFormat("en-US",{timeZone:timezone,dateStyle:"medium"}).format(new Date(row.paid_at))}));
    ...
    const pending=liveSubmissions.filter(item=>item.status==="pending").reduce((sum,item)=>sum+item.amount,0);
    const paymentAccount:PlayerPaymentAccount={totalCharges,paid,waived,pending,balance:Math.max(0,totalCharges-paid-waived)};
    ...
    const reviewBroadcast=(rosterBroadcastRows??[]).find(row=>row.broadcast_type==="roster_draft"),finalBroadcast=(rosterBroadcastRows??[]).find(row=>row.broadcast_type==="roster_final");
    const teamInfo:PlayerTeamInfo={homeUniform:uniformSettings?.dark_uniform??"Dark / Navy",awayUniform:uniformSettings?.light_uniform??"White",nextGameUniform:liveGames[0]?.uniform??"Not assigned",darkImage:publicUniformUrl(uniformSettings?.dark_image_path)||(temporaryUniformTeam?"/uniforms/trinity-travel-test-dark.png":""),lightImage:publicUniformUrl(uniformSettings?.light_image_path)||(temporaryUniformTeam?"/uniforms/trinity-travel-test-light.png":""),captain:{name:captainRow?.display_name??liveRoster.find(item=>item.role==="Captain")?.name??"Unassigned",mobile:captainRow?.mobile??""},coCaptain:{name:coCaptainRow?.display_name??liveRoster.find(item=>item.role==="Co-captain")?.name??"Unassigned",mobile:coCaptainRow?.mobile??""},rosterStage:finalBroadcast?"final":reviewBroadcast?"review":"hidden",rosterReviewDeadline:reviewBroadcast?.response_deadline??"",divisionRosters};
    const {data:broadcastRow}=invitationRow?await supabase.from("season_broadcasts").select("message,response_deadline,team_count,players_per_team,invited_count").eq("id",invitationRow.broadcast_id).maybeSingle():{data:null};
    const invitation=invitationRow&&broadcastRow?{id:invitationRow.id,seasonName:season.name,message:broadcastRow.message,response:invitationRow.response==="joining"?"joining" as const:invitationRow.response==="not_joining"?"not_joining" as const:"pending" as const,responseDeadline:broadcastRow.response_deadline??"",teamCount:broadcastRow.team_count??0,playersPerTeam:broadcastRow.players_per_team??0,invitedCount:broadcastRow.invited_count??0}:null;
