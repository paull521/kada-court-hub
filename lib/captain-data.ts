...
export async function getCaptainPortalData():Promise<CaptainPortalData>{
  ...
  const teamMap=new Map((teamRows??[]).map(row=>[row.id,row])),divisionMap=new Map((divisionRows??[]).map(row=>[row.id,row])),seasonMap=new Map((seasonRows??[]).map(row=>[row.id,row])),conferenceMap=new Map((conferenceRows??[]).map(row=>[row.id,row]));
