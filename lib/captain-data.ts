...
export type CaptainDraftCandidate={invitationId:string;registrationId:string;publicPlayerId:string;name:string;selectionStatus:string};
export type CaptainRosterPlayer={registrationId:string;name:string;publicPlayerId:string;jerseyNumber:number|null;position:string;role:string;uniformSize:string};
...
const empty:CaptainPortalData={authorized:false,activeRegistrationId:"",contexts:[],teamId:"",teamName:"",divisionName:"",seasonName:"",role:"",draftPublished:false,finalPublished:false,draftStatus:"editing",ownerNote:"",candidates:[],roster:[],requests:[],games:[],divisionGames:[],availability:[],payments:[],hasUnavailable:false};
...
export async function getCaptainPortalData():Promise<CaptainPortalData>{
  ...
  const{data:seasonRows}=seasonIds.length?await supabase.from("seasons").select("id,name").in("id",seasonIds):{data:[]};
  ...
  type CandidateRow={invitation_id:string;registration_id:string;public_player_id:string;display_name:string;selection_status:string};
  type RosterRow={registration_id:string;public_player_id:string;display_name:string;jersey_number:number|null;player_position:string|null;role_label:string;uniform_size:string|null};
