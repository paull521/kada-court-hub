...
export async function getCaptainPortalData():Promise<CaptainPortalData>{
  ...
  const contexts:CaptainContextOption[]=leaderRows.flatMap(row=>{const team=row.team_id?teamMap.get(row.team_id):undefined,division=team?divisionMap.get(team.division_id):undefined,season=division?seasonMap.get(division.season_id):undefined;return team?.active&&division&&season&&season.starts_on<=today&&season.ends_on>=today&&!season.canceled_at&&!season.archived_at?[{registrationId:row.id,teamId:team.id,teamName:team.name,divisionName:division.name,seasonName:season.name,role:row.role_label}]:[]});
