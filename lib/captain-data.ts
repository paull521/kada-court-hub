...
export async function getCaptainPortalData(scope:"full"|"home"="full"):Promise<CaptainPortalData>{
  ...
  const[{data:conferenceStatusRows},{data:leaderRows}]=await Promise.all([
    ...
    supabase.from("registrations").select("id,team_id,season_id,role_label,created_at").eq("player_id",player.id).in("role_label",["Captain","Co-captain"]).not("team_id","is",null).eq("status","active").order("created_at",{ascending:false}),
  ]);
  if(!leaderRows?.length)return empty;
  const teamIds=[...new Set(leaderRows.map(row=>row.team_id!))];
  const{data:teamRows}=await supabase.from("teams").select("id,name,division_id,active").in("id",teamIds);
  ...
  const[{data:seasonRows},{data:seasonGames}]=await Promise.all([
    seasonIds.length?supabase.from("seasons").select("id,name,conference_id,players_per_team,starts_on,ends_on,canceled_at,archived_at").in("id",seasonIds):Promise.resolve({data:[]}),
    seasonIds.length?supabase.from("games").select("season_id,starts_at,status").in("season_id",seasonIds).neq("status","canceled"):Promise.resolve({data:[]}),
  ]);
  const conferenceIds=[...new Set((seasonRows??[]).map(row=>row.conference_id))];
  const{data:conferenceRows}=conferenceIds.length?await supabase.from("conferences").select("id,name").in("id",conferenceIds):{data:[]};
  const teamMap=new Map((teamRows??[]).map(row=>[row.id,row])),divisionMap=new Map((divisionRows??[]).map(row=>[row.id,row])),seasonMap=new Map((seasonRows??[]).map(row=>[row.id,row])),conferenceMap=new Map((conferenceRows??[]).map(row=>[row.id,row])),conferenceStatus=new Map((conferenceStatusRows??[]).map((row:{conference_id:string;status:string})=>[row.conference_id,row.status]));
  const lastGameBySeason=new Map<string,string>();for(const game of seasonGames??[]){const date=game.starts_at.slice(0,10),previous=lastGameBySeason.get(game.season_id);if(!previous||date>previous)lastGameBySeason.set(game.season_id,date)}
  const contexts:CaptainContextOption[]=leaderRows.flatMap(row=>{const team=row.team_id?teamMap.get(row.team_id):undefined,division=team?divisionMap.get(team.division_id):undefined,season=division?seasonMap.get(division.season_id):undefined,lastGame=season?lastGameBySeason.get(season.id):undefined,today=new Date().toISOString().slice(0,10),lastGameAccessThrough=lastGame?new Date(`${lastGame}T00:00:00Z`):null;if(lastGameAccessThrough)lastGameAccessThrough.setUTCDate(lastGameAccessThrough.getUTCDate()+7);const gameAccessActive=lastGameAccessThrough?lastGameAccessThrough.toISOString().slice(0,10)>=today:false;return team?.active&&division&&season&&(season.ends_on>=today||gameAccessActive)&&!season.canceled_at&&!season.archived_at&&conferenceStatus.get(season.conference_id)==="active"?[{registrationId:row.id,teamId:team.id,teamName:team.name,divisionName:division.name,seasonName:season.name,role:row.role_label}]:[]});
  ...
  if(scope==="home"){
    const[{data:draft},{data:rosterRows},{data:broadcastRows},{data:workflow},{data:divisionTeams},{data:paymentRows}]=await Promise.all([
    ...
    const{data:gameRows}=workflow?.status==="final"&&divisionTeamIds.length?await supabase.from("games").select("id,home_team_id,away_team_id,starts_at,venue,court,home_uniform,away_uniform,finalized_at,status").eq("season_id",season.id).in("home_team_id",divisionTeamIds).order("starts_at"):{data:[]};
    const timezone="America/Los_Angeles",now=Date.now();
    type HomeGameRow={id:string;home_team_id:string;away_team_id:string;starts_at:string;venue:string;court:string|null;home_uniform:string|null;away_uniform:string|null;finalized_at:string|null;status:string};
    const divisionGames:CaptainGame[]=((gameRows??[]) as HomeGameRow[]).filter(row=>row.status==="scheduled"&&new Date(row.starts_at).getTime()>=now).map(row=>{const date=new Date(row.starts_at),home=row.home_team_id===team.id,parts=new Intl.DateTimeFormat("en-US",{timeZone:timezone,year:"numeric",month:"2-digit",day:"2-digit"}).formatToParts(date),part=(type:Intl.DateTimeFormatPartTypes)=>parts.find(item=>item.type===type)?.value??"";return{id:row.id,dateKey:`${part("year")}-${part("month")}-${part("day")}`,dateLabel:new Intl.DateTimeFormat("en-US",{timeZone:timezone,weekday:"long",month:"short",day:"numeric"}).format(date),time:new Intl.DateTimeFormat("en-US",{timeZone:timezone,hour:"numeric",minute:"2-digit"}).format(date),opponent:teamNames.get(home?row.away_team_id:row.home_team_id)??"Opponent",homeTeam:teamNames.get(row.home_team_id)??"Home Team",awayTeam:teamNames.get(row.away_team_id)??"Away Team",venue:row.venue,court:row.court??"",uniform:(home?row.home_uniform:row.away_uniform)??(home?"White":"Dark"),finalized:Boolean(row.finalized_at)}});
    const games=divisionGames.filter(row=>row.homeTeam===team.name||row.awayTeam===team.name),nextGame=games[0];
    const{data:availabilityRows}=nextGame?await supabase.rpc("get_team_game_availability",{p_game_id:nextGame.id}):{data:[]};
    type RosterRow={registration_id:string;public_player_id:string;display_name:string;jersey_number:number|null;player_position:string|null;jersey_name:string|null;role_label:string;uniform_size:string|null};type AvailabilityRow={registration_id:string;player_name:string;jersey_number:number|null;player_position:string;role_label:string;available:boolean;responded:boolean};type PaymentRow={registration_id:string;player_name:string;league_fee_cents:number;uniform_fee_cents:number;platform_fee_cents:number;total_charges_cents:number;paid_cents:number;waived_cents:number;pending_cents:number;balance_cents:number;payment_status:string};
    const roster=((rosterRows??[]) as RosterRow[]).map(row=>({registrationId:row.registration_id,name:row.display_name,publicPlayerId:row.public_player_id,jerseyNumber:row.jersey_number,position:row.player_position??"",jerseyName:row.jersey_name??"",role:row.role_label,uniformSize:row.uniform_size??""}));
