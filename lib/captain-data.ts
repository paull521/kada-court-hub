...
export async function getCaptainPortalData():Promise<CaptainPortalData>{
  ...
  const{data:seasonRows}=seasonIds.length?await supabase.from("seasons").select("id,name,players_per_team,starts_on,ends_on,canceled_at,archived_at").in("id",seasonIds):{data:[]};
