...
export async function getAvailableRoles():Promise<AvailableRoles>{
  ...
  const[{data:player},{data:owner}]=await Promise.all([
    ...
    supabase.from("conference_memberships").select("id").eq("profile_id",userId).eq("role","owner").limit(1).maybeSingle(),
  ]);
  let captain=false;
  if(player){const{data}=await supabase.from("registrations").select("id").eq("player_id",player.id).in("role_label",["Captain","Co-captain"]).in("status",["active","pending"]).limit(1).maybeSingle();captain=Boolean(data)}
