...
export async function updateDraftPlayerAction(_:CaptainActionState,formData:FormData):Promise<CaptainActionState>{
  ...
  const supabase=await createClient();const{error}=await supabase.rpc(detailsOnly?"captain_update_player_details":"captain_update_draft_player",detailsOnly?{p_team_id:teamId,p_registration_id:registrationId,p_jersey_number:jersey,p_position:position||null,p_uniform_size:uniformSize||null,p_jersey_name:jerseyName||null,p_assign_co_captain:assignCoCaptain}:{p_team_id:teamId,p_registration_id:registrationId,p_jersey_number:jersey,p_position:position||null,p_uniform_size:uniformSize||null,p_jersey_name:jerseyName||null,p_remove:remove});if(error)return{error:error.message};refresh();return{message:remove?"Player returned to the draft pool.":"Player information updated."};
}
