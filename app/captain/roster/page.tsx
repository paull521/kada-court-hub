...
export default async function CaptainRosterPage(){
  ...
  return <CaptainShell data={data} active="dashboard" title="Team Roster" subtitle="Build, submit, and revise your team roster.">
    ...
    {data.finalPublished&&<><details className="card captain-roster-disclosure"><summary><span><i>✓</i><span><b>Published Roster</b><small>Open the published team list.</small></span></span><strong>›</strong></summary><div className="captain-final-team">{data.roster.map(player=><div className="roster-row" key={player.registrationId}><b className="jersey">{player.jerseyNumber??"—"}</b><span className="roster-player-name"><strong>{player.name}</strong><small>{player.jerseyName?`${player.jerseyName} · `:""}{player.position||"Position not set"} · {player.uniformSize||"Size needed"}</small></span><em>{player.role}</em></div>)}</div></details><details className="card captain-roster-disclosure captain-player-details-disclosure"><summary><span><i>♙</i><span><b>Change Player Details</b><small>Jersey, jersey name, position, and uniform size.</small></span></span><strong>›</strong></summary><div><CaptainDraftRoster data={data} detailsOnly/></div></details></>}
    <details className="card captain-roster-disclosure"><summary><span><i>↔</i><span><b>Request Change</b><small>Send a team change for owner approval.</small></span></span><strong>›</strong></summary><div><CaptainRequestForm data={data} enabled={data.draftPublished}/></div></details>
