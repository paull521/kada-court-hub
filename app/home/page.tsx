...
export default async function Home(){
  ...
  return <AppShell active="home" contexts={data.contexts} activeRegistrationId={data.activeRegistrationId} notifications={data.notifications} profileNeedsAttention={data.profileNeedsAttention} paymentNeedsAttention={data.paymentNeedsAttention} teamHasUnavailable={data.teamHasUnavailable}>
    ...
    {next?<><section className="card feature-card"><div className="feature-copy"><p className="eyebrow">NEXT GAME</p><p className="feature-date">{next.dateLabel}</p><strong className="feature-time">{next.time}</strong><p className="feature-location">⌖ &nbsp;{next.venue}<br/><span>{next.court}</span></p></div><div className="matchup-logos"><div><span className="team-mark">K</span><b>{data.context.team}</b></div><strong className="versus">VS</strong><div><span className="team-mark opponent">{next.opponent.slice(0,2).toUpperCase()}</span><b>{next.opponent}</b></div></div><div className="uniform-line"><small>JERSEY COLOR</small><span className={`uniform-dot ${next.uniform.toLowerCase().includes("dark")?"dark":"white"}`}/><b>{next.uniform.toUpperCase()}</b></div></section><section className="card home-availability-card"><AvailabilityControl gameId={next.id} available={data.myAvailability}/></section></>:<section className="card empty-feature"><span>▦</span><div><p className="eyebrow">SCHEDULE</p><h2>No upcoming game yet</h2><p>Your conference owner will publish the next game here.</p></div></section>}
