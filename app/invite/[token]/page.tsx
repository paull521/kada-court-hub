...
export default async function ConferenceInvitePage({
  ...
  const [{ data: details }, { data: claims }] = await Promise.all([
    supabase.rpc("get_conference_player_invitation", { p_token: token }),
      ...
      <section className="content">
        {claims?.claims?.sub ? (
          ...
          <section className="card join-card">
            ...
            <Link className="btn secondary" href={`/sign-up?next=${encodeURIComponent(nextPath)}`}>
              Create KCH Profile
            </Link>
          </section>
        )}
