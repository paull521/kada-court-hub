...
export async function acknowledgeRulesAction(
  ...
  if (!/^[0-9a-f-]{36}$/i.test(rulesDocumentId))
    return { error: "This rules page is not available." };
  const supabase = await createClient();
  const { error } = invitationId
    ? await supabase.rpc("acknowledge_rules_and_join", {
        p_invitation_id: invitationId,
        p_rules_document_id: rulesDocumentId,
      })
    : await supabase.rpc("acknowledge_registration_rules", {
