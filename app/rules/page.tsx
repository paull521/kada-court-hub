...
export default async function RulesPage({
  ...
  if (params.invitation) {
    const { data: row } = await supabase.rpc("get_invitation_rules", {
      p_invitation_id: params.invitation,
    });
    record = (row?.[0] ?? null) as RuleRecord | null;
  } else if (params.acknowledgment) {
    const { data: rows } = await supabase.rpc("get_player_rule_acknowledgments");
    record = ((rows ?? []).find(
      (row: { acknowledgment_id: string }) => row.acknowledgment_id === params.acknowledgment,
    ) ?? null) as RuleRecord | null;
  } else if (params.registration) {
    const { data: row } = await supabase.rpc("get_registration_rules", {
      p_registration_id: params.registration,
    });
    record = (row?.[0] ?? null) as RuleRecord | null;
  } else if (params.required && data.activeRegistrationId) {
    const { data: row } = await supabase.rpc("get_registration_rules", {
  ...
  const acknowledging = Boolean(
    params.invitation || ((params.required || params.registration) && !record.acknowledged_at),
  );
  return (
    <AppShell
      active="profile"
