...
export default async function RulesPage({searchParams}:{searchParams:Promise<{invitation?:string;acknowledgment?:string;required?:string}>}){
  ...
  else if(params.required){const{data:row}=await supabase.rpc("get_required_rule_acknowledgment");record=(row?.[0]??null) as RuleRecord|null;}
  ...
  const acknowledging=Boolean((params.invitation||params.required)&&!record.acknowledged_at);
