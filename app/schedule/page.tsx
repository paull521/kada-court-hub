...
function GameRow({game,teamName}:{game:Game;teamName:string}){return <section className="card compact-upcoming-game"><time><b>{game.day}</b><span>{game.month}</span><strong>{game.date}</strong></time><div className="compact-game-main"><strong>{teamName} <span>vs</span> {game.opponent}</strong><small>⌖ {game.venue}{game.court?` · ${game.court}`:""}</small></div><div className="compact-game-side"><strong>{game.time}</strong><span><small>UNIFORM</small><i className={`uniform-dot ${game.uniform.toLowerCase().includes("dark")?"dark":"white"}`}/>{game.uniform.toUpperCase()}</span></div></section>}
...
export default async function Schedule(){
  ...
  return <AppShell active="schedule" contexts={data.contexts} activeRegistrationId={data.activeRegistrationId} notifications={data.notifications} profileNeedsAttention={data.profileNeedsAttention} paymentNeedsAttention={data.paymentNeedsAttention} teamHasUnavailable={data.teamHasUnavailable} conferenceName={data.context.conference}>
    ...
    {next?<><section className="card feature-card schedule-feature"><div className="feature-copy"><p className="eyebrow">NEXT GAME</p><p className="feature-date">{next.dateLabel}</p><strong className="feature-time">{next.time}</strong></div><div className="matchup-logos"><div><span className="team-mark">K</span><b>{data.context.team}</b></div><strong className="versus">VS</strong><div><span className="team-mark opponent">{next.opponent.slice(0,2).toUpperCase()}</span><b>{next.opponent}</b></div></div><p className="feature-venue">⌖ {next.venue}{next.court?` · ${next.court}`:""}</p><div className="uniform-line"><small>UNIFORM</small><span className={`uniform-dot ${next.uniform.toLowerCase()}`}/><b>{next.uniform.toUpperCase()}</b></div></section>{upcoming.length>0&&<><h2 className="list-label">UPCOMING GAMES</h2><div className="schedule-list">{upcoming.map(game=><GameRow game={game} teamName={data.context.team} key={game.id}/>)}</div></>}</>:<section className="card schedule-empty"><span>▦</span><h2>No games scheduled for your team</h2><p>New games will appear here as soon as the conference owner publishes them.</p></section>}
