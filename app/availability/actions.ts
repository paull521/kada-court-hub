...
export async function setAvailabilityAction(_:AvailabilityActionState,formData:FormData):Promise<AvailabilityActionState>{
  const gameId=String(formData.get("gameId")??""),response=String(formData.get("available")??"");
  if(!uuid.test(gameId)||!["yes","no"].includes(response))return{error:"Choose Yes or No."};
